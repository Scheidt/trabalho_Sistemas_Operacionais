...
int main() {
    ...
    canhaoAtira(origemCanhao0, 20); 
    canhaoMovendo[0] = 1;
    moveCanhaoD2E(origemCanhao0, 23, 0); 
    canhaoMovendo[0] = 0; 
    Sleep(1000); 
    ...
    canhaoMovendo[1] = 1;
    moveCanhaoD2E(origemCanhao1, 23, 1); 
    canhaoAtira(origemCanhao0, 20); 
    canhaoMovendo[1] = 0; 
    Sleep(1000);  
    ...
    int heliX = 50, heliY = 10; // Initial position of helicopter

    while(1) { 
        if(GetAsyncKeyState(VK_LEFT)) {
            heliX = (heliX > 0) ? heliX - 1 : 0; 
            helicopteroD2E(heliX, heliY); // Changed to draw the helicopter facing left
        } else if(GetAsyncKeyState(VK_RIGHT)) {
            heliX = (heliX < 115) ? heliX + 1 : 115;
            helicopteroE2D(heliX, heliY); // Draw the helicopter facing right
        } else if(GetAsyncKeyState(VK_UP)) {
            heliY = (heliY > 0) ? heliY - 1 : 0;
            helicopteroE2D(heliX, heliY); // Keep the last direction or choose a default one
        } else if(GetAsyncKeyState(VK_DOWN)) {
            heliY = (heliY < 20) ? heliY + 1 : 20;
            helicopteroE2D(heliX, heliY); // Keep the last direction or choose a default one
        } else {
            helicopteroE2D(heliX, heliY); // Default drawing if no arrow keys are pressed
        }

        Sleep(100); 
        apaga_helicopteroE2D(heliX, heliY); // You may also need a function to erase the helicopter drawn by helicopteroD2E
